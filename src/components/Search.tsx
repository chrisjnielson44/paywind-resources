...
export function Search() {
  ...
  return (
    <>
      <button
        type="button"
        className="group flex h-6 w-6 items-center justify-center sm:justify-start md:h-auto md:w-80 md:flex-none md:rounded-lg md:py-2.5 md:pl-4 md:pr-3.5 md:text-sm md:ring-1 md:ring-slate-200 md:hover:ring-slate-300 lg:w-96 dark:md:bg-zinc-900 dark:md:ring-inset dark:md:ring-white/10 dark:md:hover:bg-slate-700/40 dark:md:hover:ring-slate-500"
        {...buttonProps}
      >
        <SearchIcon className="h-5 w-5 flex-none fill-slate-400 group-hover:fill-slate-500 md:group-hover:fill-slate-400 dark:fill-white" />
        <span className="sr-only md:not-sr-only md:ml-2 md:text-slate-500 md:dark:text-white">
          Search docs
        </span>
        {modifierKey && (
          <kbd className="ml-auto hidden font-medium text-slate-400 md:block dark:text-slate-500">
            <kbd className="font-sans dark:text-white">{modifierKey}</kbd>
            <kbd className="font-sans dark:text-white">K</kbd>
          </kbd>
        )}
      </button>
